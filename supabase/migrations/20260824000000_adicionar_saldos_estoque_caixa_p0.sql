...
ALTER TABLE public.lancamentos
    ADD COLUMN IF NOT EXISTS estoque_inicial NUMERIC(14,2),
    ADD COLUMN IF NOT EXISTS estoque_final NUMERIC(14,2),
    ADD COLUMN IF NOT EXISTS caixa_inicial NUMERIC(14,2),
    ADD COLUMN IF NOT EXISTS caixa_final NUMERIC(14,2);

ALTER TABLE public.lancamentos
    DROP CONSTRAINT IF EXISTS lancamentos_estoque_inicial_check,
    DROP CONSTRAINT IF EXISTS lancamentos_estoque_final_check,
    DROP CONSTRAINT IF EXISTS lancamentos_caixa_inicial_check,
    DROP CONSTRAINT IF EXISTS lancamentos_caixa_final_check;

ALTER TABLE public.lancamentos
    ADD CONSTRAINT lancamentos_estoque_inicial_check CHECK (estoque_inicial >= 0),
    ADD CONSTRAINT lancamentos_estoque_final_check CHECK (estoque_final >= 0),
    ADD CONSTRAINT lancamentos_caixa_inicial_check CHECK (caixa_inicial >= 0),
    ADD CONSTRAINT lancamentos_caixa_final_check CHECK (caixa_final >= 0);

CREATE OR REPLACE FUNCTION public.herdar_saldos_lancamento_substituto_p0()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
    IF NEW.substitui_lancamento_id IS NULL THEN
        RETURN NEW;
    END IF;
    ...
    IF FOUND THEN
        NEW.estoque_inicial := v_original.estoque_inicial;
        NEW.estoque_final := v_original.estoque_final;
        NEW.caixa_inicial := v_original.caixa_inicial;
        NEW.caixa_final := v_original.caixa_final;
    END IF;
...
DROP TRIGGER IF EXISTS trg_herdar_saldos_lancamento_substituto_p0
    ON public.lancamentos;

CREATE TRIGGER trg_herdar_saldos_lancamento_substituto_p0
BEFORE INSERT ON public.lancamentos
FOR EACH ROW
EXECUTE FUNCTION public.herdar_saldos_lancamento_substituto_p0();
...
BEGIN
    IF p_empresa_id IS NULL
        ...
        OR p_estoque_inicial IS NULL
        OR p_estoque_inicial < 0
        OR p_estoque_final IS NULL
        OR p_estoque_final < 0
        OR p_caixa_inicial IS NULL
        OR p_caixa_inicial < 0
        OR p_caixa_final IS NULL
        OR p_caixa_final < 0
        OR p_itens IS NULL
    ...
    INSERT INTO public.lancamentos (
        ...
        estoque_inicial, estoque_final, caixa_inicial, caixa_final,
...
REVOKE ALL ON FUNCTION public.herdar_saldos_lancamento_substituto_p0()
FROM PUBLIC, anon, authenticated;
